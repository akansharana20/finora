...
export const VatOverview: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* TOP PRIORITY VAT PERIOD & LIABILITY HEADER */}
      {currentPeriod ? (
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <span className="text-lg font-bold text-slate-800 block">
              £{Number(liveCalculation?.box1 || 0).toFixed(2)}
            </span>
            <span className="text-xs text-slate-500">{liveCalculation?.transactionCount?.invoices || 0} sales invoices</span>
          </div>

          <div className="text-right">
            <span className="text-xs font-semibold text-slate-400 block mb-1">Estimated VAT Liability (Box 5)</span>
            <span className="text-2xl font-bold text-blue-600 block">
              £{Number(liveCalculation?.box5 || 0).toFixed(2)}
            </span>
            <button
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start space-x-3.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">No VAT Obligations on Record</h3>
              <p className="text-xs text-slate-500 mt-1 max-w-xl">
                This company has no open or historical VAT obligations recorded yet. In Phase 2, connect to HMRC Making Tax Digital to synchronize your official filing obligations.
              </p>
            </div>
          </div>
          <div className="text-xs font-semibold text-slate-600 bg-slate-50 px-3.5 py-2 rounded-lg border border-slate-200 shrink-0">
            Awaiting HMRC MTD Connection
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900 text-white rounded-xl p-5 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        {currentPeriod ? (
          ...
          >
            Review & File Return ({currentPeriod.periodKey})
          </button>
        ) : (
          <span className="text-xs font-semibold bg-slate-800 text-slate-400 px-4 py-2 rounded-lg shrink-0 border border-slate-700">
            Awaiting VAT Period
          </span>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <span className="text-base font-bold text-slate-900">£{Number(liveCalculation?.box1 || 0).toFixed(2)}</span>
          </div>
          <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
            <span className="font-bold text-slate-500 block mb-1">Box 4: VAT reclaimed on purchases</span>
            <span className="text-base font-bold text-slate-900">£{Number(liveCalculation?.box4 || 0).toFixed(2)}</span>
          </div>
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <span className="font-bold text-blue-700 block mb-1">Box 5: Net VAT Payable / Reclaimable</span>
            <span className="text-lg font-bold text-blue-700">£{Number(liveCalculation?.box5 || 0).toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
          ...
          {obligations && obligations.length > 0 ? (
            ...
            <div className="py-8 text-center text-slate-400 text-xs">
              No VAT obligations on record for this company.
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
          ...
          {returns && returns.length > 0 ? (
            ...
            <div className="py-8 text-center text-slate-400 text-xs">
              No submitted or draft VAT returns yet.
            </div>
          )}
        </div>
      </div>
